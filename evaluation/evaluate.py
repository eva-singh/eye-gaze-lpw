...
def main():
    ...
    all_detections = []  # (confidence, is_tp) across the whole dataset -- for mAP
        ...
        for images, gt_boxes, names in loader:
            ...
            for pred, gt_box_tensor in zip(predictions, gt_boxes):
                ...
                for i, score in enumerate(pred_scores):
                    is_tp = (i == best_idx and best_iou >= args.iou_threshold)
    ...
    f = f1(p, r)
    ap50 = compute_ap(all_detections, num_gt, args.iou_threshold)
    ...
    with open(out_path, "w", newline="") as f_out:
        ...
        writer.writerow(["model", "precision", "recall", "f1", "map50", "fps", "conf_threshold", "iou_threshold", "num_images"])
        writer.writerow([args.model, f"{p:.4f}", f"{r:.4f}", f"{f:.4f}", f"{ap50:.4f}", f"{fps:.2f}", args.conf_threshold, args.iou_threshold, len(dataset)])
